...
--------------------------------------------*/
function getAbs(a,b){
    return Math.abs(a-b); //함수를 호출한 실행부가 결과를 가지고 후출한 곳으로 되돌아감
}
...
function getRandom(n){
    ...
    return Math.floor((n+1)*r); //소수점을 버린다 =floor
}
...
function getLastDate(yy,mm){
    ...
    d.setFullYear(yy);
    d.setMonth(mm+1); //일단 다음달로 넘어간다 왜? 0을 대입하여 이전 달의 마지막 일을 고하려고
    d.setDate(0);//다시 이전달로 넘어오면서 마지막 날로 세팅된다
